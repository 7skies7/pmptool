    ...
    public function fetchUserComments()
    {
        ...
        if(User::isRole(6) || User::isRole(7))
        {
            ...
            if(count($projects) > 0)
            {
                ...
                // dd($taskid);
                $comments = TaskComment::with('users')->with('project')->select('task_id','task_comment','task_hours','task_completion','id','created_at','created_by')->where('is_deleted',0)->limit(20)->get();
            }
        }
        return $comments;
    }
